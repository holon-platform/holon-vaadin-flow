...
[source, java]
----
include::{examplesdir}/com/holonplatform/vaadin/flow/examples/ExampleNavigation16.java[tag=auth1,indent=0]
----
<1> The `View1` routing target class, and so the `some/path` route path, can be accessed only by an authenticated user 
<1> The `View2` routing target class, and so the `some/path` route path, can be accessed only by an authenticated user and a `login` redirect URI is specified and will be used to redirect the user navigation when the authentication is not available
...
[source, java]
----
include::{examplesdir}/com/holonplatform/vaadin/flow/examples/ExampleNavigation16.java[tag=auth2,indent=0]
----
