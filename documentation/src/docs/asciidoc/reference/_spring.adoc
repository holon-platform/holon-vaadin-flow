...
* One for the `UnauthorizedNavigationException`: see link:{apidir}/com/holonplatform/vaadin/flow/navigator/errors/UnauthorizedNavigationError.html[UnauthorizedNavigationError^].

* One for the `ForbiddenNavigationException`: see link:{apidir}/com/holonplatform/vaadin/flow/navigator/errors/ForbiddenNavigationError.html[ForbiddenNavigationError^].

The `holon.vaadin.navigator.errors.enabled` application configuration property can be setted to `false` to disable the error components registration.
