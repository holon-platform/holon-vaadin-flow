 ...
 * the License.
 */
package com.holonplatform.vaadin.flow.internal.components.fix;
...
/**
 * Temporary TimePicker implementantion to avoid NPE in setValue of default Vaadin impl.
 * 
 * TODO remove when bug resolved
 */
@SuppressWarnings("serial")
@JavaScript("frontend://timepickerConnector.js")
public class TimePicker extends GeneratedVaadinTimePicker<TimePicker, LocalTime> implements HasSize, HasValidation {

	private static final SerializableFunction<String, LocalTime> PARSER = valueFromClient -> {
		return valueFromClient == null || valueFromClient.isEmpty() ? null : LocalTime.parse(valueFromClient);
	};
	 ...
	 *
	 * @param time the pre-selected time in the picker
	 */
	 ...
	 *
	 * @param label the label describing the time picker
	 * @see #setLabel(String)
	 */
	...
	/**
	 * Convenience constructor to create a time picker with a {@link ValueChangeListener}.
	 *
	 * @param listener the listener to receive value change events
	 * @see #addValueChangeListener(HasValue.ValueChangeListener)
	 */
	public TimePicker(ValueChangeListener<ComponentValueChangeEvent<TimePicker, LocalTime>> listener) {
		this();
	...
	public void setValue(LocalTime value) {
		LocalTime truncatedValue = (value == null) ? null : value.truncatedTo(ChronoUnit.MILLIS);
	 ...
	 * <p>
	 * This property is not synchronized automatically from the client side, so the returned value may not be the same
	 * as in client side.
	 * </p>
	 *
	...
	/**
	 * Sets the {@code step} property of the time picker using duration. It specifies the intervals for the displayed
	 * items in the time picker dropdown and also the displayed time format.
	 * <p>
	 ...
	 * <p>
	 * If the step is less than 60 seconds, the format will be changed to {@code hh:mm:ss} and it can be in
	 * {@code hh:mm:ss.fff} format, when the step is less than 1 second.
	 * <p>
	 ...
	 * {@link com.vaadin.flow.component.HasValue.ValueChangeEvent} to be fired if some parts (eg. seconds) is discarded
	 * from the value.</em>
	 *
	...
	public void setStep(Duration step) {
		...
		long stepAsMilliseconds = step.getSeconds() * 1000 + (long) (step.getNano() / 1E6);
		...
		if (MILLISECONDS_IN_A_DAY % stepAsMilliseconds != 0 && MILLISECONDS_IN_AN_HOUR % stepAsMilliseconds != 0) {
			throw new IllegalArgumentException(
					"Given step " + step.toString() + " does not divide evenly a day or an hour.");
		}
	...
	private void initConnector() {
		...
		runBeforeClientResponse(ui -> ui.getPage()
				.executeJavaScript("window.Vaadin.Flow.timepickerConnector.initLazy($0)", getElement()));
	}

	/**
	 * Set the Locale for the Time Picker. The displayed time will be formatted by the browser using the given locale.
	 * <p>
	 * By default, the locale is {@code null} until the component is attached to an UI, and then locale is set to
	 * {@link UI#getLocale()}, unless a locale has been explicitly set before that.
	 * <p>
	 ...
	 * <p>
	 * <em>NOTE: only the language + country/region codes are used</em>. This means that the script and variant
	 * information is not used and supported. <em>NOTE: timezone related data is not supported.</em> <em>NOTE: changing
	 * the locale does not cause a new {@link com.vaadin.flow.component.HasValue.ValueChangeEvent} to be fired.</em>
	 *
	 * @param locale the locale set to the time picker, cannot be [@code null}
	 */
	public void setLocale(Locale locale) {
		...
		runBeforeClientResponse(ui -> getElement().callFunction("$connector.setLocale", bcp47LanguageTag.toString()));
	}
	 ...
	 * <p>
	 * By default, the locale is {@code null} until the component is attached to an UI, and then locale is set to
	 * {@link UI#getLocale()}, unless {@link #setLocale(Locale)} has been explicitly called before that.
	 *
	 ...
	 *
	 * @param min the String value to set
	 */
	@Override
	 ...
	 * Set the minimum values for the date picker.
	 *
	 * @param max the String value to set
	 */
	@Override
	 ...
	 * <p>
	 * This is a shorthand for {@link Locale#getAvailableLocales()} where all locales without the
	 * {@link Locale#getLanguage()} have been filtered out, as the browser cannot localize the time for those.
	 *
	 * @return a stream of the available locales that are supported by the time picker component
