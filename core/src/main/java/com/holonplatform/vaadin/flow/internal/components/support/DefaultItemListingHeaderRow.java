...
public class DefaultItemListingHeaderRow<P> implements EditableItemListingRow<P> {
	...
	public Optional<ItemListingCell> getCell(P property) {
		...
		if (column != null) {
			try {
				return Optional.ofNullable(new DefaultItemListingHeaderCell(row.getCell(column)));
	...
	public ItemListingCell join(Collection<P> properties) {
		...
		return new DefaultItemListingHeaderCell(row.join(columns.toArray(new Column<?>[columns.size()])));
	}
